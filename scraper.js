// New imports for serverless compatibility
const axios = require("axios");
const cheerio = require("cheerio");

// --- IMPORTANT: Only import puppeteer-core for deployment ---
// We will conditionally use @sparticuz/chromium for the executable
const puppeteer = require('puppeteer-core');
const chromium = require('@sparticuz/chromium'); 

// Removed: puppeteer-extra and StealthPlugin are too large and complex for Vercel Serverless

// --- Site-Specific Function for AsuraScans Series ---
// (No changes needed for Cheerio/Axios based function)
...
async function scrapeAsuraChapterImages(chapterUrl) {
    ...
    // --- Recommended memory-saving args from @sparticuz/chromium ---
    const LAUNCH_ARGS = [
        ...chromium.args,
        '--no-sandbox',
        '--disable-setuid-sandbox',
        '--single-process',          // Use single process mode to save RAM
        '--disable-gpu',
    ...
    try {
        ...
        // 1. Launch with serverless-compatible settings
        browser = await puppeteer.launch({
            args: LAUNCH_ARGS,
            defaultViewport: chromium.defaultViewport,
            executablePath: await chromium.executablePath(), // Dynamically finds the Chromium path in Vercel's /tmp
        ...
        // 2. Add resource blocking to speed up page load and save memory
        await page.setRequestInterception(true);
        page.on('request', (req) => {
            // Block fonts, images (unless they are the content), and styles to speed up page.goto()
        ...
        // 3. Use 'domcontentloaded' to wait for less time than 'load'
        await page.goto(chapterUrl, { waitUntil: 'domcontentloaded', timeout: 30000 }); // Reduced timeout

        await page.keyboard.press('Escape'); // close popups
        await new Promise(r => setTimeout(r, 500)); // Reduced wait time

        // 4. Optimized scroll function call
        ...
        if (page) {
             // Screenshots won't be saved on Vercel filesystem, but the error log is more valuable
             console.log("Puppeteer failed. Check Vercel logs for full error details."); 
        }
        return [];
    } finally {
...
// Optimized scroll function: Reduced fixed delay
async function fullAutoScroll(page) {
    ...
    while (true) {
        ...
        await new Promise(r => setTimeout(r, 300)); // Reduced wait from 1500ms to 300ms
    }
}


// --- Other Site-Specific Functions (No changes needed) ---
